...
def generate_3d_flipped_spiral(n:int, noise=0.2, radial_scaling=1.5, z_start=0, z_range=20):
    X = np.zeros((2 * n, 3))
    y = np.zeros(2 * n, dtype=int)

    for i in range(2):
        theta = np.linspace(0, 4 * np.pi, n)
        r = radial_scaling * theta
        ...
        if i % 2 == 0:
            X[i * n:(i + 1) * n, 2] = np.linspace(z_start, z_start + z_range, n)  # Upward slope
        else:
            X[i * n:(i + 1) * n, 2] = np.linspace(z_start + z_range, z_start, n)  # Downward slope
...
def visualize_spiral():
    ...
    plt.tight_layout()
    plt.savefig('mockdataset.png')
    plt.show()


n = 2000
X, y = generate_3d_flipped_spiral(n)
visualize_spiral()
...
##Doesn't fit to the data, produces NaNs
#kernel = Kernel()
...
for losses, label, color in zip(
    [losses_laplacian_downstream, losses_lle_downstream, losses_ltsa_downstream, losses_vgae_downstream, losses_vae_downstream, losses_pca_downstream, 
     #losses_diffmap_downstream, 
     losses_ae_downstream, losses_n2v_downstream, losses_mds_downstream, losses_isomap_downstream],
    ['Laplacian Eigenmaps','LLE','LTSA', 'VGAE', 'VAE', 'PCA', 
     #'Diffusion Map', 
     'AE', 'Node2Vec', 'MDS', 'Isomap'],
    [colors['Laplacian Eigenmaps'], colors['LLE'], colors['LTSA'], colors['VGAE'], colors['VAE'], colors['PCA'], 
     #colors['Diffusion Map'], 
     colors['AE'], colors['Node2Vec'], colors['MDS'], colors['Isomap']]
):

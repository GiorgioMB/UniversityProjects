...
from metrics import *
from WOS.preprocessing import load_data
...
# If WOS/plos folder does not exist, create it
if not os.path.exists('WOS/plots'):
    os.makedirs('WOS/plots')
...
plt.savefig('WOS/plots/kl_loss.png')
...
# Save the plot to plots folder
plt.savefig('WOS/plots/recon_loss.png')
...
# Save the plot to plots folder
plt.savefig('WOS/plots/node2vec_loss.png')
...
# Save the plot to plots folder
plt.savefig('WOS/plots/tsne_vgae.png')
...
# Save the plot to plots folder
plt.savefig('WOS/plots/tsne_n2v.png')
...
# Save the plot to plots folder
plt.savefig('WOS/plots/tsne_vae.png')
...
# Save the plot to plots folder
plt.savefig('WOS/plots/tsne_pca.png')
...
classifier1 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier2 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier3 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier4 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier5 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier6 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier7 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier8 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier9 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier10 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
classifier11 = nn.Sequential(nn.Linear(latent, 32), nn.ReLU(), nn.Linear(32, number_classes), nn.LogSoftmax(dim=1)).to(device)
...
train_labels = torch.tensor(y[train_mask_np].values, dtype=torch.long).to(device)
test_ltsa_embeddings = torch.tensor(embeddings_ltsa[test_mask], dtype=torch.float).to(device)
test_labels = torch.tensor(y[test_mask_np].values, dtype=torch.long).to(device)
...
print("Epoch range length:", len(range(1, epochs + 1)))
print("Loss array lengths:")
print("PCA Downstream:", len(losses_pca_downstream))
print("MDS Downstream:", len(losses_mds_downstream))
print("Isomap Downstream:", len(losses_isomap_downstream))
# Repeat for other loss arrays
...
for losses, label, color in zip(
    ...
    ['Laplacian Eigenmaps','LLE','LTSA', 'VGAE', 'VAE', 'PCA', 'Diffusion Map', 'AE', 'Node2Vec', 'Isomap'],
    [colors['Laplacian Eigenmaps'], colors['LLE'], colors['LTSA'], colors['VGAE'], colors['VAE'], colors['PCA'], colors['Diffusion Map'], colors['AE'], colors['Node2Vec'], colors['Isomap']]
):
...
# Save the plot to plots folder
plt.savefig('WOS/plots/downstream_task_loss.png')

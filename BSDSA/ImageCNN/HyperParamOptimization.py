...
model = Sequential([
    Rescaling(1./255),
    Conv2D(best_params['num_filters'], best_params['kernel_size'], activation='relu'),
    MaxPooling2D(pool_size=(2, 2)),
    Conv2D(best_params['num_filters'], best_params['kernel_size'], activation='relu'),
    MaxPooling2D(pool_size=(2, 2)),
    Conv2D(best_params['num_filters'], best_params['kernel_size'], activation='relu'),
    ...
    MaxPooling2D(pool_size=(2, 2)),
    Flatten(),
    Dense(128, kernel_regularizer=tf.keras.regularizers.l1(best_params['l1_lambda']), activation='relu'),
    Dense(64, kernel_regularizer=tf.keras.regularizers.l1(best_params['l1_lambda']), activation='relu'),
    Dense(32, kernel_regularizer=tf.keras.regularizers.l1(best_params['l1_lambda']), activation='relu'),
    Dense(16, activation='relu'),
    Dense(6)
])
model.compile(

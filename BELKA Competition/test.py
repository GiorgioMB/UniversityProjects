...
class MoleculeGraphDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> Tuple[Data, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        return (*graphs, local, global_) ##Return the graphs, one-hot encoded protein name and the target
...
def collate_fn(batch):
    ...
    return (*graphs, local_vector, global_vector)
...
if __name__ == '__main__':
    ...
    train_path = 'test.parquet'
    ##Check that the file exists
    if not os.path.exists(train_path):
        print("File not found")
        raise FileNotFoundError
    ##Note: for now we're only using 2Million 0s
    con = duckdb.connect()
    ...
    processed_ddf = ddf.map_partitions(process_and_replace_smiles_columns, meta = sample_df) ##HOPEFULLY THIS WORKS, I HATE DASK
    ...
    counter = 0
    with torch.no_grad():
        for data, local_vector, global_vector in test_loader:
